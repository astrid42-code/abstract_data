...
namespace ft
{
	...
	class rbt_set
	{
		public:
			...
			typedef typename ft::rbt_it_set<node, key_cmp>									iterator;
			typedef typename ft::rbt_const_it_set<node, key_cmp>							const_iterator;
		...
		public:
			...
			rbt_set() : _root(NULL), _cmp(key_cmp()), _alloc_rbt(alloc_node()), _size(0) { }

			rbt_set(const rbt_set &x){
				_root = x._root;
				_cmp = x._cmp;
			...
			rbt_set & operator=(const rbt_set &x)
			{
			...
			iterator	find(const value_type &x) const
			{
				...
				while (tmp)
				{
					if (value_compare(x, tmp->pair))
						tmp = tmp->left_child;
					else if (value_compare(tmp->pair, x))
						tmp = tmp->right_child;
					else
			...
			const_iterator	const_find(const value_type &x) const
			{
				node_ptr tmp = _root;
			...
			iterator	lower_bound(const value_type & x) const
			{
				...
				{
					if (!(value_compare(tmp_b.get_node()->pair, x)))
						return (tmp_b);
				}
			...
			const_iterator	const_lower_bound(const value_type & x)
			{
			...
			iterator	upper_bound(const value_type & x) const
			{
				...
				for (;tmp_b != tmp_e; tmp_b++)
				{
					if (value_compare(x, tmp_b.get_node()->pair))
						return (tmp_b);
				}
			...
			const_iterator	const_upper_bound(const value_type & x)
			{
				const_iterator	tmp_b = begin();
				const_iterator	tmp_e = end();
			...
			size_type	erase(const value_type& x)
			{
				...
				while (tmp_b && tmp_b->pair != x)
				{
					if (tmp_b && value_compare(x, tmp_b->pair))
						tmp_b = tmp_b->left_child;
					else if (tmp_b && value_compare(tmp_b->pair, x))
